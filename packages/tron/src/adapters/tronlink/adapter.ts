...
export interface TronLinkAdapterConfig extends BaseAdapterConfig {
    /**
     * 检测 TronLink 钱包是否存在的超时时间（毫秒）
     * 默认 30 * 1000ms
     */
    checkTimeout?: number;
    /**
     * 是否在移动端通过 DeepLink 打开 TronLink App
     * 默认 true
     */
    openTronLinkAppOnMobile?: boolean;
    /**
     * DApp 图标地址，用于在移动端打开 TronLink App 时展示
     */
    dappIcon?: string;
    /**
     * DApp 名称，用于在移动端打开 TronLink App 时展示
     */
    dappName?: string;
}
...
export class TronLinkAdapter extends Adapter {
    ...
    /**
     * 切换到目标链；若当前链与目标链相同则立即成功
     * 可用 chainId：
     * - 主网 Mainnet: 0x2b6653dc
     * - 测试网 Shasta: 0x94a9059e
     * - 测试网 Nile: 0xcd8690dc
    ...
    /**
     * 通过定时轮询检测钱包是否存在；检测到或超时后 promise 才会 resolve
     * @returns 是否检测到钱包
     */
    ...
    /**
     * 在 TronLink App 中轮询检查钱包是否就绪
     * 钱包就绪后更新状态并触发 connect 事件
     */
