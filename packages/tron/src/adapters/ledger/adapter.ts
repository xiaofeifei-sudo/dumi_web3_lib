import {
    Adapter,
    AdapterState,
    isInBrowser,
    WalletNotFoundError,
    WalletDisconnectedError,
    WalletSignMessageError,
    WalletSignTransactionError,
    WalletConnectionError,
    WalletDisconnectionError,
    WalletReadyState,
} from '@tronweb3/tronwallet-abstract-adapter';

import type { Transaction, SignedTransaction, AdapterName } from '@tronweb3/tronwallet-abstract-adapter';
import type { Account, LedgerUtils, LedgerWalletConfig } from './LedgerWallet';
import { LedgerWallet } from './LedgerWallet';

/**
 * Ledger 适配器（Tron）
 * - 负责以统一的适配器接口封装 Ledger 硬件钱包的连接、断开、签名能力
 * - 通过 WebHID 能力检测浏览器可用性（navigator.hid），从而设置 readyState
 * - 内置状态机：NotFound → Disconnect → Connected；对外通过事件进行通知
 * - 事件：
 *   - 'connect': 成功连接并选定地址后触发，携带 address
 *   - 'disconnect': 主动断开或清理后触发
 *   - 'stateChanged': 适配器状态变化时触发（AdapterState）
 *   - 'error': 出现任何异常时触发（Wallet*Error 类型）
 * - 错误映射：
 *   - WalletNotFoundError：在不支持 WebHID 或找不到 Ledger 时
 *   - WalletConnectionError：连接失败或用户未按要求准备设备/进入 Tron 应用
 *   - WalletDisconnectedError：在未连接状态下请求签名
 *   - WalletSignMessageError / WalletSignTransactionError：签名过程发生错误
 */
const isSupportedLedger = () => !!(globalThis.navigator && (globalThis.navigator as any).hid);

export type LedgerAdapterConfig = LedgerWalletConfig;
export const LedgerAdapterName = 'Ledger' as AdapterName<'Ledger'>;

export class LedgerAdapter extends Adapter {
    name = LedgerAdapterName;
    url = 'https://www.ledger.com/';
    icon =
        'data:image/png;base64,';

    private _readyState: WalletReadyState = WalletReadyState.Loading;
    private _state: AdapterState = AdapterState.NotFound;
    private _connecting: boolean;
    private _address: string | null;
    private config;

    private _wallet: LedgerWallet;
    constructor(config: LedgerAdapterConfig = {}) {
        super();
        this._connecting = false;
        this._address = null;
        this.config = config;
        this._wallet = new LedgerWallet(config);
        if (isSupportedLedger()) {
            this._readyState = WalletReadyState.Found;
            this._state = AdapterState.Disconnect;
        } else {
            this._readyState = WalletReadyState.NotFound;
        }
    }

    get address() {
        return this._address;
    }

    get state() {
        return this._state;
    }

    get connecting() {
        return this._connecting;
    }

    get readyState() {
        return this._readyState;
    }
    get ledgerUtils(): LedgerUtils {
        return {
            getAccounts: this._wallet.getAccounts,
            getAddress: this._wallet.getAddress,
        };
    }

    /**
     * 连接到 Ledger 并选定账户地址
     * - options.account：可直接传入已选定账户（包含 index 与 address），将跳过硬件交互和选择流程
     * - 默认行为：弹出提示（beforeConnect 未提供时）要求用户准备好设备并进入 Tron 应用，然后加载初始账户并展示选择弹窗
     * - 成功后：
     *   - 设置当前地址（this._address）
     *   - 状态切换为 Connected
     *   - 派发 'connect' 与 'stateChanged' 事件
     * - 失败时：
     *   - 按类型抛出 WalletNotFoundError / WalletConnectionError，并派发 'error' 事件
     */
    async connect(options?: { account: Omit<Account, 'path'> }): Promise<void> {
        try {
            if (this.connected || this.connecting) return;
            if (this.state === AdapterState.NotFound) {
                if (this.config.openUrlWhenWalletNotFound !== false && isInBrowser()) {
                    window.open(this.url, '_blank');
                }
                throw new WalletNotFoundError();
            }
            this._connecting = true;
            try {
                await this._wallet.connect(options);
            } catch (e: any) {
                // @todo: 验证/确认 Ledger 的隐藏错误信息
                // if (e.message?.includes('Incorrect length (0x6700)')) {
                //     // 此错误码通常表示设备上未打开正确的应用（例如未进入 Tron 应用）
                //     // 参考：https://www.reddit.com/r/ledgerwallet/comments/mxcxo3/can_someone_help_with_the_problem_ledger_device/
                //     throw new WalletConnectionError('请准备好 Ledger 并进入 Tron 应用。');
                // }
                throw new WalletConnectionError(`${e.message}.`);
            }
            this._address = this._wallet.address;
            this._state = AdapterState.Connected;
            this.emit('connect', this.address || '');
            this.emit('stateChanged', this._state);
        } catch (error: any) {
            this.emit('error', error);
            throw error;
        } finally {
            this._connecting = false;
        }
    }

    /**
     * 断开当前 Ledger 连接
     * - 清理当前选定地址与状态
     * - 派发 'disconnect' 与 'stateChanged' 事件
     * - 若清理失败，抛出 WalletDisconnectionError 并派发 'error' 事件
     */
    async disconnect(): Promise<void> {
        if (this.state !== AdapterState.Connected) {
            return;
        }
        try {
            this._wallet.disconnect();
            this._state = AdapterState.Disconnect;
            this._address = null;
            this.emit('disconnect');
            this.emit('stateChanged', this._state);
        } catch (e: any) {
            this.emit('error', e);
            throw new WalletDisconnectionError(e.message);
        }
    }

    /**
     * 使用 Ledger 对 Tron 事务进行签名
     * - 前置要求：必须处于 Connected 状态，否则抛出 WalletDisconnectedError
     * - 成功返回 SignedTransaction，其中包含（或追加）签名数组
     * - 失败抛出 WalletSignTransactionError，并派发 'error' 事件
     */
    async signTransaction(transaction: Transaction): Promise<SignedTransaction> {
        try {
            if (this.state !== AdapterState.Connected) {
                throw new WalletDisconnectedError();
            }
            try {
                const signedTransaction = await this._wallet.signTransaction(transaction);
                return signedTransaction;
            } catch (e: any) {
                throw new WalletSignTransactionError(e.message);
            }
        } catch (error: any) {
            this.emit('error', error);
            throw error;
        }
    }

    /**
     * 使用 Ledger 对文本消息进行个人签名（personal message）
     * - 前置要求：必须处于 Connected 状态，否则抛出 WalletDisconnectedError
     * - 返回签名十六进制字符串
     * - 失败抛出 WalletSignMessageError，并派发 'error' 事件
     */
    async signMessage(message: string): Promise<string> {
        try {
            if (this.state !== AdapterState.Connected) {
                throw new WalletDisconnectedError();
            }
            try {
                const signedResponse = await this._wallet.signPersonalMessage(message);
                return signedResponse;
            } catch (error: any) {
                throw new WalletSignMessageError(error?.message, error);
            }
        } catch (error: any) {
            this.emit('error', error);
            throw error;
        }
    }
}
