...
Ant Design Web3 officially provides adapters for multiple frameworks such as `wagmi` and `ethers` to adapt to Ethereum. We recommend using `pelican-web3-lib-wagmi`. It is an Ant Design Web3 Ethereum adapter based on [wagmi 2.x](https://wagmi.sh/). It provides the ability to connect to Ethereum and other EVM compatible chains for the components of `pelican-web3-lib`.

If you are using wagmi 1.x, you can check [pelican-web3-lib-wagmi@1.2.0 documentation](https://github.com/ant-design/ant-design-web3/blob/f7c9d51086f82b13a9cf94353b999348e17001de/packages/web3/src/wagmi/index.md).
...
If you want to use `ethers` in your project, we provide the `pelican-web3-lib-ethers` adapter, which is compatible with the latest `v6` and `v5` versions. You can [view the documentation](../ethereum-ethersjs/index.md) to use it.
...
Based on [Chain](./types#chain)[] adds the `wagmiChain` field to support chain configuration. Import it in a way similar to `import { Mainnet } from 'pelican-web3-lib-wagmi';`.
