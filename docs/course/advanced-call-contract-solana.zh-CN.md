...
DApp 与区块链合约的交互是 DApp 的核心功能之一。实现这一点并不困难，但是需要写一些逻辑。在这个示例中，我们将演示如何使用 Pelican Web3 连接 Solana 网络，并调用 Memo 合约。
...
在上面的代码中，我们并没有使用任何 `pelican-web3-lib-solana` 中的内容，实际调用位于 `writeMemo` 函数。
