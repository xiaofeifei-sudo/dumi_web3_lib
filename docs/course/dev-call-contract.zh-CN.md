...
`balanceOf` 是用来获取某一个地址下有多少个 NFT 的合约方法。所以我们还需要用到 `pelican-web3-lib` 提供的 `useAccount` Hook 来获取当前连接的账户地址。然后将账户地址作为 `balanceOf` 方法的参数传入，这样就可以获取到当前账户地址下有多少个 NFT 了。如果不出意外，你会得到 `0` 的结果。
