...
Pelican Web3 is a Web3 React component library based on [Ant Design](https://ant.design/), it provides a series of components that can help you quickly build decentralized applications (DApp).

You can use Pelican Web3 in any of the following ways:

- Use UI components directly through `pelican-web3-lib`, such as `ConnectButton`, `Address`, etc. You can choose any way you like to connect to the blockchain, such as [ether](https://docs.ethers.org/v6/), [viem](https://viem.sh/) and [web3.js](https://web3js.org/). And any other chain's SDK, including non-EVM compatible blockchains, can also use Pelican Web3 in this way.
- Use the adapter provided by us with `pelican-web3-lib` to quickly connect to various blockchains without having to handle the related logic of connecting to the blockchain yourself.
  - Connect to EVM-compatible chains based on `pelican-web3-lib-wagmi` or `pelican-web3-lib-ethers` and `pelican-web3-lib-eth-web3js`.
  - Connect to Solana based on `pelican-web3-lib-solana`.
  - Connect to Bitcoin based on `pelican-web3-lib-bitcoin`.
  - Connect to Sui based on `pelican-web3-lib-sui`.
...
- Use only the official adapter, such as `pelican-web3-lib-solana`, and then completely customize the UI components, consume the capabilities provided by the adapter through [Web3ConfigProvider](http://localhost:8000/components/web3-config-provider).

## Try it out

Here are examples of using Pelican Web3 through `pelican-web3-lib-wagmi`:
...
If you want to further try how to use Pelican Web3 in your own project, or want to create a project based on Pelican Web3 from 0, you can continue reading [Quick Start](quick-start.md), wish you a pleasant surfing in Web3! 🌊🌊🌊

## Why use it?

Why use Pelican Web3?

- 🎨 Provides high-quality UI components based on Ant Design. [Ant Design](https://github.com/ant-design/ant-design) is a high-quality UI basic component set that supports flexible theme customization and has nearly 100,000 GitHub stars and many users, which gives Pelican Web3 a basic quality guarantee.
