...
<NormalInstallDependencies packageNames="antd pelican-web3-lib pelican-web3-lib-wagmi wagmi viem @tanstack/react-query" save="true"></NormalInstallDependencies>

<br />

## Use UI components

`pelican-web3-lib` is a pure UI component, you can use it directly, for example, you can use `Address`:
...
`pelican-web3-lib` does not provide the ability to connect to the blockchain itself. It is a pure UI component library. We designed it from the beginning to consider that it can be adapted to different blockchains in the future. But we officially implemented an adapter `pelican-web3-lib-wagmi` based on [wagmi](https://wagmi.sh/). It has built-in connection capabilities with EVM compatible chains, and you can connect to Ethereum through it.

In `pelican-web3-lib-wagmi`, we expose a component `WagmiWeb3ConfigProvider`, which encapsulates wagmi's `WagmiConfig`.

As long as you use `WagmiWeb3ConfigProvider` as `WagmiConfig`, the Pelican Web3 components in your application can automatically connect to the blockchain.
...
- In [Next.js](https://nextjs.org/), based on its build mechanism, you may encounter the following errors when introducing packages such as `pelican-web3-lib`:
...
You may encounter the following error when using `pelican-web3-lib` in [Remix](https://remix.run/) (Not the Remix IDE of Ethereum):
